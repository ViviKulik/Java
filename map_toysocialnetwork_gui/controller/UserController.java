...
//            System.out.println(user.getId());
    ...
    private void showFriendList(Utilizator user){
        try{
            FXMLLoader loader = new FXMLLoader();
            loader.setLocation(getClass().getResource("/friendlist-view.fxml"));
            AnchorPane root =(AnchorPane)loader.load();

            Stage dialogStage = new Stage();
            dialogStage.setTitle("Your friends");
            dialogStage.initModality(Modality.WINDOW_MODAL);
            Scene scene = new Scene(root);
            dialogStage.setScene(scene);

            FriendListController FriendListRequest = loader.getController();
            FriendListRequest.setService(service, dialogStage, user.getId());
